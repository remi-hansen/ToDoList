...
//  ToDoList
//
//  Created by Remi Pacifico Hansen on 10/17/22.
//
...
//        don''t try to update if toDos.id == Nil, which it should never be. But if a developer tries to call this in the wrong place, we're covered.
        guard toDo.id != nil else {return}
//        copy toDo (a constant) into a newToDo (var) so we can update the isCompleted Property
        var newToDo = toDo
        newToDo.isCompleted.toggle() //flips false to try and true to false
